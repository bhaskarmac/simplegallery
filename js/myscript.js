$(function () {
  ...
  function displayImages(imageJSON) {
    ...
    for (var i = 0; i < imageJSON.length; i++) {
      singleImage =
        ...
        '<img class="img-responsive imgLink" alt="' +
        imageJSON[i].name +
        '" src="' +
        imageJSON[i].imgSource +
        '">' +
        "</li>";
  ...
  jQuery(document).on("click", ".navigationControls", function (e) {
    ...
    var imgSrc = jQuery("ul.row li:nth-child(" + imgNumber + ") img").attr(
      "src"
    );
    var imgName = jQuery("ul.row li:nth-child(" + imgNumber + ") img").attr(
      "alt"
    );
